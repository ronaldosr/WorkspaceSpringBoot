...
//		PessoaVO vo = DozerConverter.parseObject(pessoaRepository.save(entity), PessoaVO.class);
	...
	public List<PessoaVO> listarTodos(Pageable pageable) {
		var entities = pessoaRepository.findAll(pageable).getContent();
		return DozerConverter.parseListObjects(entities, PessoaVO.class);
	}
	
	public Page<PessoaVO> listarTodosHateoas(Pageable pageable) {
	...
	public Page<PessoaVO> listarPessoasPorNome(String primeiroNome, Pageable pageable) {
		var page = pessoaRepository.listarPessoaPorNome(primeiroNome, pageable);
		return page.map(this::convertToPessoaVO);
	}
